...
class MultaTableMap extends TableMap
{
     ...
     */
    const CLASS_NAME = '.Map.MultaTableMap';
     ...
     */
    const COL_SOCIO_ID = 'multa.socio_id';

    /**
     * the column name for the cantidad field
     */
    const COL_CANTIDAD = 'multa.cantidad';

    /**
     * the column name for the comentario field
     */
    const COL_COMENTARIO = 'multa.comentario';

    /**
     * the column name for the created_at field
     */
    const COL_CREATED_AT = 'multa.created_at';

    /**
     * the column name for the updated_at field
     */
    const COL_UPDATED_AT = 'multa.updated_at';
  ...
  array (
    ...
    {
        return $withPrefix ? MultaTableMap::CLASS_DEFAULT : MultaTableMap::OM_CLASS;
    }
     ...
     * @return array           (Multa object, last column rank)
     */
    public static function populateObject($row, $offset = 0, $indexType = TableMap::TYPE_NUM)
    {
        $key = MultaTableMap::getPrimaryKeyHashFromRow($row, $offset, $indexType);
        if (null !== ($obj = MultaTableMap::getInstanceFromPool($key))) {
            // We no longer rehydrate the object, since this can cause data loss.
            // See http://www.propelorm.org/ticket/509
            // $obj->hydrate($row, $offset, true); // rehydrate
            $col = $offset + MultaTableMap::NUM_HYDRATE_COLUMNS;
        } else {
            $cls = MultaTableMap::OM_CLASS;
            /** @var Multa $obj */
            $obj = new $cls();
            $col = $obj->hydrate($row, $offset, false, $indexType);
            MultaTableMap::addInstanceToPool($obj, $key);
        }
    ...
    public static function populateObjects(DataFetcherInterface $dataFetcher)
    {
        ...
        while ($row = $dataFetcher->fetch()) {
            $key = MultaTableMap::getPrimaryKeyHashFromRow($row, 0, $dataFetcher->getIndexType());
            if (null !== ($obj = MultaTableMap::getInstanceFromPool($key))) {
                // We no longer rehydrate the object, since this can cause data loss.
                // See http://www.propelorm.org/ticket/509
    ...
    public static function addSelectColumns(Criteria $criteria, $alias = null)
    {
        if (null === $alias) {
            ...
            $criteria->addSelectColumn(MultaTableMap::COL_CREATED_AT);
    ...
    {
        return Propel::getServiceContainer()->getDatabaseMap(MultaTableMap::DATABASE_NAME)->getTable(MultaTableMap::TABLE_NAME);
    }
    ...
    {
        $dbMap = Propel::getServiceContainer()->getDatabaseMap(MultaTableMap::DATABASE_NAME);
        if (!$dbMap->hasTable(MultaTableMap::TABLE_NAME)) {
            $dbMap->addTableObject(new MultaTableMap());
        }
    }

    /**
     * Performs a DELETE on the database, given a Multa or Criteria object OR a primary key value.
     *
     ...
     */
     public static function doDelete($values, ConnectionInterface $con = null)
     {
        ...
        if ($values instanceof Criteria) {
            ...
            $criteria->add(MultaTableMap::COL_ID, (array) $values, Criteria::IN);
        }

        $query = MultaQuery::create()->mergeWith($criteria);

        if ($values instanceof Criteria) {
            MultaTableMap::clearInstancePool();
        } elseif (!is_object($values)) { // it's a primary key, or an array of pks
            foreach ((array) $values as $singleval) {
                MultaTableMap::removeInstanceFromPool($singleval);
            }
        }
    ...
    {
        return MultaQuery::create()->doDeleteAll($con);
    }

    /**
     * Performs an INSERT on the database, given a Multa or Criteria object.
     *
     * @param mixed               $criteria Criteria or Multa object containing data that is used to create the INSERT statement.
    ...
    public static function doInsert($criteria, ConnectionInterface $con = null)
    {
        ...
        if ($criteria instanceof Criteria) {
            ...
            $criteria = $criteria->buildCriteria(); // build Criteria from Multa object
        }

        if ($criteria->containsKey(MultaTableMap::COL_ID) && $criteria->keyContainsValue(MultaTableMap::COL_ID) ) {
